...
Flea At Home 现在不附带任何官方游戏素材。为了让跳蚤动画正常显示，您需要自行提供视频或音频文件：

1. **准备素材**  
   - 视频格式：`.webm` VP8 1920x1080px with Alpha通道
   - 音频格式：`.wav`  

2. **放置路径**  
   - 将视频文件放入 `FleaAtHome` 文件夹
   - 文件名可以自定义，但建议与 Config 中的文件名对应  

3. **修改 Config**（可选）  
   - 打开 `BepInEx/config/com.gcx.fleaathome.cfg`  
   - 修改以下字段以匹配你的视频文件名和路径：
     ```ini
     [Media]
